        ...
        for i in range(0, len(new_docs), batch_size):
            ...
            for doc_id, doc in tqdm_async(
                ...
                try:
                    ...
                    # Log the token count for this document's chunks
                    input_tokens = sum(len(chunk["content"].split()) for chunk in chunks.values())
                    output_tokens = input_tokens  # Adjust output tokens if necessary (same for now)
                    self.update_token_count(input_tokens, output_tokens)
                    self.logger.info(f"Processed {len(chunks)} chunks for document {doc_id}. "
